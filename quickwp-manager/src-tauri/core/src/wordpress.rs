...
add_action('init', function () {
    ...
    // Or this site's front page, logged in: the app's live preview.
    if (!empty($_GET['nexora_front'])) {
        $target = home_url('/');
    }
...
/// Lets WordPress answer on Nexora's own server port as well as its address.
///
/// A site's stored address goes through the 80/443 edge. When that edge is not
/// Nexora's -- HTTPS not set up, or another local tool holding the ports -- the
/// app reaches the site on `<domain>:18089` instead, and WordPress would
/// redirect every request straight back to the stored address. Reached on that
/// port, it takes the address the request arrived on: links, redirects, login
/// cookies, and URLs saved in content all follow it. No other request is
/// touched.
const PORT_MU_PLUGIN: &str = r#"<?php
/**
 * Plugin Name: Nexora local port
 * Description: Generated by Nexora. Lets the site answer on Nexora's own server port.
 */
(function () {
    $host = isset($_SERVER['HTTP_HOST']) ? strtolower((string) $_SERVER['HTTP_HOST']) : '';
    if (!preg_match('/^([a-z0-9.-]+):__PORT__$/', $host, $m)) {
        return;
    }
    $base = 'http://' . $host;
    $name = preg_quote($m[1], '~');
    $follow = function () use ($base) {
        return $base;
    };
    add_filter('pre_option_home', $follow);
    add_filter('pre_option_siteurl', $follow);
    force_ssl_admin(false);

    // Images and links saved in posts carry the stored address.
    ob_start(function ($out) use ($name, $base) {
        foreach (headers_list() as $h) {
            if (stripos($h, 'content-type:') === 0 && stripos($h, 'text/html') === false) {
                return $out;
            }
        }
        $swapped = preg_replace('~https?://' . $name . '(?![\w.:-])~i', $base, $out);
        return $swapped === null ? $out : $swapped;
    });
})();
"#;

/// Write the port mu-plugin into a WordPress site. Rewritten every time, like
/// the login one, so an older copy never outlives an upgrade.
pub fn ensure_port_mu_plugin(site: &Site) -> Result<()> {
    let docroot = std::path::PathBuf::from(&site.docroot);
    if !docroot.join("wp-content").is_dir() {
        return Ok(()); // not a WordPress site
    }
    let path = docroot.join("wp-content/mu-plugins/nexora-port.php");
    paths::mkdir_p(path.parent().unwrap())?;
    let body = PORT_MU_PLUGIN.replace("__PORT__", &crate::ports::NGINX.to_string());
...
/// A one-time link that lands on the front page, logged in as the
/// administrator: the site the way the person building it sees it, admin bar
/// and all.
pub fn magic_login_front(site: &Site, url: &str) -> Result<String> {
    Ok(format!("{}&nexora_front=1", magic_login_to(site, url, "", None)?))
}
...
// WordPress's own cookie, not one assembled by hand: it also records the login
// session the cookie names, and wp_validate_auth_cookie() refuses a cookie no
// session holds -- so a hand-made one looked right and never logged anyone in.
WP_CLI::print_value(wp_generate_auth_cookie($u->ID, time() + 3600, 'auth'));"#,
        user = user
    );
    let mut c = wp(site)?;
