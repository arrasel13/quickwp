...
//! `adminer.quickwp.<tld>` through a PHP pool that already exists. There is no
...
//!
//!   * QuickWP's MySQL root account has no password by design (see
...
//! form at it, so a token is required. It arrives once in the URL QuickWP
...
///
/// Under `quickwp.<tld>` rather than `adminer.<tld>` so the name cannot collide
/// with a site someone actually wants to call "adminer".
pub fn host(tld: &str) -> String {
    format!("adminer.quickwp.{tld}")
}
...
/// generated code, and a stale copy from an older QuickWP is a bug that would
...
pub fn url(tld: &str, mysql_port: u16, database: &str, token: &str) -> String {
    ...
    format!(
        "https://{}/?server={}&username=root&db={}&quickwp_auth={}",
        host(tld),
        server,
        urlencode(database),
        token,
...
// Generated by QuickWP. Edits are overwritten -- change core/src/adminer.rs.

// Adminer answers on a real hostname, so any page in the browser could point a
// form at it. The token is what stops that. It arrives once in the URL QuickWP
...
$given = '';
if (isset($_GET['quickwp_auth'])) {
    $given = $_GET['quickwp_auth'];
} elseif (isset($_COOKIE['quickwp_auth'])) {
    $given = $_COOKIE['quickwp_auth'];
}
if ($expected === '' || !hash_equals($expected, (string) $given)) {
    ...
    echo '<p>This database browser only opens from QuickWP, which puts a one-time key in the address. ';
    echo 'Open it from a site&rsquo;s <b>Database</b> tab.</p>';
    exit;
}
if (isset($_GET['quickwp_auth'])) {
    setcookie('quickwp_auth', $expected, [
        'path' => '/',
        'httponly' => true,
        'samesite' => 'Lax',
    ]);
...
function adminer_object() {
    ...
    // "connect as root". QuickWP's MySQL root has no password by design: the
    ...
    class QuickWpAdminer extends \Adminer\Adminer {
        ...
        function headers() {
            ...
            // QuickWP shows Adminer inside its own Database tab, so the frame
...
mod tests {
    ...
    fn the_host_cannot_collide_with_a_site_called_adminer() {
        assert_eq!(host("test"), "adminer.quickwp.test");
    ...
    fn the_url_encodes_the_port_colon_and_the_database_name() {
        ...
        assert!(u.contains("quickwp_auth=abc123"), "{u}");
        assert!(u.starts_with("https://adminer.quickwp.test/"), "{u}");
    }
