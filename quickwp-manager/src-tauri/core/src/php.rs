...
/// The php.ini directives QuickWP exposes.
///
...
pub fn set_ini(minor: &str, key: &str, value: &str) -> Result<()> {
    if !INI_WHITELIST.iter().any(|(k, _)| *k == key) {
        return Err(Error::other(format!(
            "`{key}` is not an editable directive. QuickWP exposes: {}",
            INI_WHITELIST
                .iter()
                .map(|(k, _)| *k)
                .collect::<Vec<_>>()
...
fn write_pool_config(minor: &str, port: u16) -> Result<PathBuf> {
    ...
    let body = format!(
        r#"; Generated by QuickWP. Edits here are overwritten.
[global]
...
pub fn start_pool(sup: &Supervisor, minor: &str) -> Result<u16> {
    ...
    let conf = write_pool_config(minor, port)?;
